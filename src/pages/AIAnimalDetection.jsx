...
import {
  ...
  FaCheckCircle, FaExclamationTriangle, FaCopy, FaHeartbeat,
  FaTimes, FaExpand, FaHistory, FaKeyboard, FaClock, FaBolt,
  FaChartBar, FaChevronDown, FaImage, FaList, FaCode,
  FaShieldAlt, FaMicrochip, FaFingerprint, FaDog, FaCircle,
  FaInfoCircle, FaChartPie, FaFileExport, FaSearchPlus,
  FaArrowRight, FaLayerGroup, FaCrosshairs, FaWaveSquare,
  FaStar, FaDatabase
} from 'react-icons/fa';
...
/* ═══════════════════ CONFIDENCE BAR ═══════════════════ */
const ConfidenceBar = ({ value, color = '#3b82f6' }) => (
  ...
  const particles = useMemo(
    () =>
      Array.from({ length: 12 }, (_, i) => ({
        id: `p-${i}`, x: Math.random() * 1400, y: Math.random() * 700 + 50,
        size: Math.random() * 16 + 6, delay: Math.random() * 8, duration: Math.random() * 10 + 8,
      })),
    []
  );
...
// Push to history (in-      // Push to history (in-component + localStorage + backend)
      ...
      const historyEntry = {
        ...
        elapsed,
        modelType:   'animal',   // ← change to 'plant' in AIPlantDetection.jsx
      ...
      toast.success(`Detected ${preds.length} animal(s) in ${elapsed}ms!`);
    } catch (e) {
      console.error(e);
      setDetectionResult({ status: 'error', predictions: [] });
      toast.error('Animal detection failed — check backend.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const drawDetections = useCallback(async () => {
    ...
    const drawBox = (p, style) => {
      const x = p.x1 * sx,
        ...
        w = (p.x2 - p.x1) * sx,
        h = (p.y2 - p.y1) * sy;
      const color = colorMap[p.class] || '#3b82f6';
  ...
  const downloadJSON = () => {
    ...
    a.download = `animal-detections-${Date.now()}.json`;
    a.click();
  ...
  const downloadCSV = () => {
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `animal-detections-${Date.now()}.csv`;
    a.click();
  ...
  const downloadAnnotatedImage = () => {
    ...
    const a = document.createElement('a');
    a.download = `animal-annotated-${Date.now()}.png`;
  ...
  /* ── Analysis mode badge ── */
  const modeBadge = {
    general: { icon: <FaPaw />, cls: 'text-blue-600 bg-blue-50 border-blue-200' },
    health: { icon: <FaHeartbeat />, cls: 'text-red-600 bg-red-50 border-red-200' },
    behavior: { icon: <FaEye />, cls: 'text-purple-600 bg-purple-50 border-purple-200' },
  }[detectionType];
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-blue-50 via-white to-sky-50 overflow-hidden">
      {/* ── Backgrounds ── */}
      <GridBackground />
      <div className="pointer-events-none absolute -top-40 -right-40 w-[55rem] h-[55rem] rounded-full bg-blue-300/10 blur-[140px]" />
      <div className="pointer-events-none absolute -bottom-40 -left-40 w-[55rem] h-[55rem] rounded-full bg-sky-300/10 blur-[140px]" />
      <div className="pointer-events-none absolute top-1/3 left-1/2 -translate-x-1/2 w-[70rem] h-[40rem] rounded-full bg-indigo-200/5 blur-[160px]" />
      {particles.map((p) => (
      ...
      >
        ...
        <motion.div
          ...
          className="flex flex-wrap items-center justify-between gap-3 mb-6
                     bg-white/80 backdrop-blur-xl rounded-2xl border border-blue-100/80
                     shadow-sm shadow-blue-100/30 px-5 py-3"
        >
          <div className="flex items-center gap-4">
            <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs
                              font-bold border ${modeBadge.cls}`}>
              {modeBadge.icon}
              {detectionType.charAt(0).toUpperCase() + detectionType.slice(1)} Mode
            </span>
            <LiveClock />
            {processingTime > 0 && (
        ...
        <motion.div variants={fadeUp} className="text-center mb-8">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-5 py-2 rounded-full
                       ...
                       text-xs font-bold mb-4 border border-blue-200/50 shadow-sm"
          >
            <FaShieldAlt className="text-blue-500" />
            YOLOv8 Animal Detection Engine
          ...
          <p className="text-gray-500 text-sm md:text-base max-w-2xl mx-auto">
            Upload or capture an image to detect animals with AI-powered object detection.
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <motion.div variants={fadeUp} className="xl:col-span-3 space-y-5">
            {/* Upload Card */}
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg border border-blue-100 p-5 overflow-hidden">
              ...
              <div
                ...
                className={`relative rounded-2xl border-2 transition-all duration-300 overflow-hidden ${
                  dragActive
                    ? 'border-blue-400 bg-blue-50 shadow-lg shadow-blue-100/50 scale-[1.02]'
                    : preview
              ...
              >
                {dragActive && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="absolute inset-0 bg-blue-500/5 backdrop-blur-[1px] flex items-center
                ...
                {!preview && !usingCamera ? (
                  ...
                  >
                    <div className="w-16 h-16 mx-auto mb-3 rounded-2xl bg-gradient-to-br from-blue-100 to-sky-100
              ...
              <div className="grid grid-cols-2 gap-2 mt-4">
                {!usingCamera ? (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={startCamera}
                    className="bg-gradient-to-r from-gray-50 to-blue-50 hover:from-blue-50 hover:to-blue-100
                               text-blue-700 py-2.5 rounded-xl flex items-center justify-center gap-2
                               text-sm font-semibold border border-blue-200 transition-all duration-300"
                  >
                    <FaCamera /> Camera
                  </motion.button>
                ) : (
                  <motion.button
                    ...
                    className="bg-gradient-to-r from-blue-600 to-sky-600 text-white py-2.5 rounded-xl
            ...
            {/* Controls Card */}
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg border border-blue-100 p-5">
              <h2 className="text-sm font-bold flex items-center gap-2 text-gray-800 mb-4">
                <div className="w-6 h-6 rounded-lg bg-blue-100 flex items-center justify-center">
                  <FaLayerGroup className="text-blue-600 text-[10px]" />
                </div>
                Controls
              </h2>
              ...
              <div className="space-y-5">
                  ...
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      confidenceThreshold > 70 ? 'bg-green-100 text-green-700'
                        : confidenceThreshold > 40 ? 'bg-blue-100 text-blue-700'
                ...
                {/* Analysis Mode */}
                <div>
                  <label className="text-xs font-semibold text-gray-600 mb-2 block">Analysis Mode</label>
                  <div className="grid grid-cols-3 gap-1.5">
                    {[
                      { value: 'general', label: 'General', icon: <FaPaw className="text-[10px]" />, color: 'blue' },
                      { value: 'health', label: 'Health', icon: <FaHeartbeat className="text-[10px]" />, color: 'red' },
                      { value: 'behavior', label: 'Behavior', icon: <FaEye className="text-[10px]" />, color: 'purple' },
                    ].map((m) => {
                      const active = detectionType === m.value;
                      const colorMap2 = {
                        blue: active ? 'bg-blue-600 text-white border-blue-600' : '',
                        red: active ? 'bg-red-500 text-white border-red-500' : '',
                        purple: active ? 'bg-purple-600 text-white border-purple-600' : '',
                      };
                      return (
                        <button
                          key={m.value}
                          onClick={() => setDetectionType(m.value)}
                          className={`px-2 py-2 text-[10px] rounded-lg border font-semibold
                                      flex items-center justify-center gap-1 transition-all duration-200 ${
                                        active
                                          ? `${colorMap2[m.color]} shadow-sm`
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg border border-blue-100 overflow-hidden">
              <button
                onClick={() => setShowHistory((v) => !v)}
                className="w-full p-4 flex items-center justify-between hover:bg-blue-50/50 transition"
              >
                <h2 className="text-sm font-bold flex items-center gap-2 text-gray-800">
                  <div className="w-6 h-6 rounded-lg bg-blue-100 flex items-center justify-center">
                    <FaHistory className="text-blue-600 text-[10px]" />
                  </div>
                  History
                        ...
                        detectionHistory.map((h) => (
                          <div
                            ...
                            className="flex items-center justify-between p-2.5 rounded-xl border
                                       bg-gradient-to-r from-gray-50 to-blue-50/30 text-xs"
                          >
                            <div>
                              <div className="font-bold text-gray-700">{h.count} animal(s)</div>
          ...
          <motion.div variants={fadeUp} className="xl:col-span-5">
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg border border-blue-100 p-5 overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold flex items-center gap-2 text-gray-800">
                  <div className="w-8 h-8 rounded-lg bg-blue-100 flex items-center justify-center">
                    <FaSearchPlus className="text-blue-600 text-sm" />
                  </div>
                  Preview
                  {filteredPreds.length > 0 && (
                    <span className="bg-blue-100 text-blue-700 text-[10px] font-bold px-2.5 py-0.5 rounded-full">
              ...
              <div
                ref={containerRef}
                className="relative w-full bg-gradient-to-br from-gray-50 to-blue-50/30
              ...
              >
                {usingCamera ? (
                    ...
                    <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-black/60 to-transparent rounded-b-2xl">
                      <div className="flex items-center justify-center gap-3">
                        <motion.button
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={capturePhoto}
                          className="bg-white text-blue-600 px-6 py-2.5 rounded-full shadow-lg
                  ...
                  <div className="text-center py-16">
                    <div className="w-20 h-20 mx-auto mb-4 rounded-3xl bg-gradient-to-br from-blue-100 to-sky-100
                                    flex items-center justify-center shadow-inner border border-blue-200/50">
                      <FaPaw className="text-blue-400 text-3xl" />
                    </div>
                    ...
                    >
                      ...
                      >
                        <div className="relative">
                          ...
                          <FaPaw className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600 text-xs" />
                        </div>
                        <div>
                          <div className="font-bold text-gray-800 text-sm">Analyzing image…</div>
                          <div className="text-[10px] text-gray-400">Running YOLOv8 inference</div>
                        </div>
                      </motion.div>
                    </motion.div>
                  )}
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-lg border border-blue-100 p-5 overflow-hidden">
              ...
              {!detectionResult ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-gray-100 to-blue-50
                  ...
                  <AnimatePresence mode="wait">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-sm text-gray-800">
                                {filteredPreds.length} animal(s) detected
                              </div>
                              <div className="text-[10px] text-gray-500">
                                {detectionResult.predictions?.length || 0} total •{' '}
                                {filteredPreds.length} above {confidenceThreshold}% threshold
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    {resultTab === 'charts' && (
                      <motion.div
                        ...
                        className="space-y-5"
                      >
                        <div className="rounded-2xl p-4 border bg-gradient-to-br from-white to-blue-50/30">
                          <h4 className="text-xs font-bold text-gray-700 mb-3 flex items-center gap-2">
                            <FaChartPie className="text-blue-500" /> Class Distribution
                          </h4>
                        ...
                        <div className="rounded-2xl p-4 border bg-gradient-to-br from-white to-blue-50/30">
                          <h4 className="text-xs font-bold text-gray-700 mb-3 flex items-center gap-2">
                            <FaChartBar className="text-blue-500" /> Top Confidences
                          </h4>
                            ...
                            <motion.button
                              ...
                              className={`w-full flex items-center gap-3 border-2 rounded-2xl px-4 py-3
                                         text-left transition-all duration-300 group ${
                                           p.id === highlightId
                                             ? 'border-blue-400 bg-blue-50/80 shadow-lg shadow-blue-100/50 scale-[1.01]'
                      ...
                      >
                        <div className="bg-gray-950 rounded-2xl p-4 border border-gray-800 overflow-hidden">
                          ...
                          <pre className="text-[10px] text-green-400 font-mono overflow-auto max-h-96
                            ...
                            {JSON.stringify(
                              { status: detectionResult.status, count: filteredPreds.length, predictions: filteredPreds },
                              null,
                              2
                            )}
                          </pre>
        ...
        <motion.div variants={fadeUp} className="mt-8">
          <motion.button
            ...
            className="w-full bg-white/90 backdrop-blur-sm rounded-2xl shadow-sm p-5
                       border border-blue-100 flex items-center justify-between
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-5">
                  {[
                    ...
                    },
                    {
                      ...
                      title: 'Endpoint',
                      value: 'POST /api/detect?model=animal',
        ...
        <motion.div variants={fadeUp} className="text-center mt-8 pb-6">
          <div className="flex items-center justify-center gap-3 text-xs text-gray-400">
            <span className="flex items-center gap-1.5">
              <FaShieldAlt className="text-blue-500" /> Animal Detection Studio v2.0
            </span>
            <span>•</span>
            ...
            >
              ...
              {filteredPreds.length > 0 && (
                <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm rounded-xl
                  ...
                  {filteredPreds.length} detection(s) • {uniqueCount} unique class(es)
                </div>
              )}
            </motion.div>
          </motion.div>
